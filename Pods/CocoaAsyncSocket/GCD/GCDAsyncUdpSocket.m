...
// https://github.com/robbiehanson/CocoaAsyncSocket/wiki/ARC
#endif
...
enum GCDAsyncUdpSocketFlags {
  ...
  kIPv4Deactivated =
      1 << 6,  // If set, socket4 was closed due to bind or connect on IPv6.
  kIPv6Deactivated =
      1 << 7,  // If set, socket6 was closed due to bind or connect on IPv4.
  ...
  kSock4CanAcceptBytes = 1 << 12,  // If set, we know socket4 can accept bytes.
                                   // If unset, it's unknown.
  kSock6CanAcceptBytes = 1 << 13,  // If set, we know socket6 can accept bytes.
                                   // If unset, it's unknown.
  kForbidSendReceive = 1 << 14,    // If set, no new send or receive operations
                                   // are allowed to be queued.
  kCloseAfterSends =
      1 << 15,          // If set, close as soon as no more sends are queued.
  kFlipFlop = 1 << 16,  // Used to alternate between IPv4 and IPv6 sockets.
#if TARGET_OS_IPHONE
  kAddedStreamListener =
      1 << 17,  // If set, CFStreams have been added to listener thread
#endif
};
...
/**
 * The GCDAsyncUdpSendPacket encompasses the instructions for a single
*send/write.
**/
...
- (id)initWithDelegate:(id)aDelegate
  ...
  if ((self = [super init])) {
    ...
    if (sq) {
      NSAssert(
          sq != dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0),
          @"The given socketQueue parameter must not be a concurrent queue.");
      NSAssert(
          sq != dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0),
          @"The given socketQueue parameter must not be a concurrent queue.");
      NSAssert(
          sq != dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),
          @"The given socketQueue parameter must not be a concurrent queue.");
      ...
      socketQueue =
          dispatch_queue_create([GCDAsyncUdpSocketQueueName UTF8String], NULL);
    }

    // The dispatch_queue_set_specific() and dispatch_get_specific() functions
    // take a "void *key" parameter.
    ...
    // Specifically an ivar that is explicitly named for our purpose to make the
    // code more readable.
    //
    // However, it feels tedious (and less readable) to include the "&" all the
    // time:
    // dispatch_get_specific(&IsOnSocketQueueOrTargetQueueKey)
    //
    // So we're going to make it so it doesn't matter if we use the '&' or not,
    // by assigning the value of the ivar to the address of the ivar.
    // Thus: IsOnSocketQueueOrTargetQueueKey ==
...
- (void)dealloc {
  ...
  if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey)) {
    ...
    dispatch_sync(socketQueue, ^{ [self closeWithError:nil]; });
  }
...
- (id)delegate {
  if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey)) {
    ...
    dispatch_sync(socketQueue, ^{ result = delegate; });
...
- (void)setDelegate:(id)newDelegate synchronously:(BOOL)synchronously {
  dispatch_block_t block = ^{ delegate = newDelegate; };
...
- (dispatch_queue_t)delegateQueue {
  if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey)) {
    ...
    dispatch_sync(socketQueue, ^{ result = delegateQueue; });
...
- (BOOL)isIPv4Enabled {
  ...
  dispatch_block_t block = ^{ result = ((config & kIPv4Disabled) == 0); };
...
- (BOOL)isIPv6Enabled {
  ...
  dispatch_block_t block = ^{ result = ((config & kIPv6Disabled) == 0); };
...
- (BOOL)isIPv4Preferred {
  ...
  dispatch_block_t block = ^{ result = (config & kPreferIPv4) ? YES : NO; };
...
- (BOOL)isIPv6Preferred {
  ...
  dispatch_block_t block = ^{ result = (config & kPreferIPv6) ? YES : NO; };
...
- (uint16_t)maxReceiveIPv4BufferSize {
  ...
  dispatch_block_t block = ^{ result = max4ReceiveSize; };
...
- (uint32_t)maxReceiveIPv6BufferSize {
  ...
  dispatch_block_t block = ^{ result = max6ReceiveSize; };
...
- (id)userData {
  ...
  dispatch_block_t block = ^{ result = userData; };
...
- (void)notifyDidConnectToAddress:(NSData *)anAddress {
  ...
  if (delegateQueue &&
      [delegate respondsToSelector:@selector(udpSocket:didConnectToAddress:)]) {
...
- (void)notifyDidNotConnect:(NSError *)error {
  ...
  if (delegateQueue &&
      [delegate respondsToSelector:@selector(udpSocket:didNotConnect:)]) {
    id theDelegate = delegate;
...
- (void)notifyDidSendDataWithTag:(long)tag {
  ...
  if (delegateQueue &&
      [delegate respondsToSelector:@selector(udpSocket:didSendDataWithTag:)]) {
    id theDelegate = delegate;
...
- (void)notifyDidNotSendDataWithTag:(long)tag dueToError:(NSError *)error {
  ...
  if (delegateQueue &&
      [delegate respondsToSelector:@selector(udpSocket:
...
- (void)notifyDidReceiveData:(NSData *)data
                 fromAddress:(NSData *)address
           withFilterContext:(id)context {
  LogTrace();

  SEL selector =
      @selector(udpSocket:didReceiveData:fromAddress:withFilterContext:);
...
- (void)notifyDidCloseWithError:(NSError *)error {
  ...
  if (delegateQueue &&
      [delegate respondsToSelector:@selector(udpSocketDidClose:withError:)]) {
    id theDelegate = delegate;
...
- (NSError *)gaiError:(int)gai_error {
  NSString *errMsg = [NSString stringWithCString:gai_strerror(gai_error)
                                        encoding:NSASCIIStringEncoding];
  NSDictionary *userInfo =
      [NSDictionary dictionaryWithObject:errMsg
                                  forKey:NSLocalizedDescriptionKey];

  return [NSError errorWithDomain:@"kCFStreamErrorDomainNetDB"
                             code:gai_error
                         userInfo:userInfo];
}
...
- (NSError *)errnoErrorWithReason:(NSString *)reason {
  ...
  if (reason)
    userInfo = [NSDictionary
        dictionaryWithObjectsAndKeys:errMsg, NSLocalizedDescriptionKey, reason,
                                     NSLocalizedFailureReasonErrorKey, nil];
  else
    userInfo = [NSDictionary
        dictionaryWithObjectsAndKeys:errMsg, NSLocalizedDescriptionKey, nil];

  return
      [NSError errorWithDomain:NSPOSIXErrorDomain code:errno userInfo:userInfo];
}
...
- (NSError *)sendTimeoutError {
  NSString *errMsg = NSLocalizedStringWithDefaultValue(
      @"GCDAsyncUdpSocketSendTimeoutError", @"GCDAsyncUdpSocket",
      [NSBundle mainBundle], @"Send operation timed out", nil);
...
- (NSError *)socketClosedError {
  NSString *errMsg = NSLocalizedStringWithDefaultValue(
      @"GCDAsyncUdpSocketClosedError", @"GCDAsyncUdpSocket",
      [NSBundle mainBundle], @"Socket closed", nil);
  ...
  return [NSError errorWithDomain:GCDAsyncUdpSocketErrorDomain
                             code:GCDAsyncUdpSocketClosedError
                         userInfo:userInfo];
}
...
- (BOOL)preOp:(NSError **)errPtr {
  ...
  {
    if (errPtr) {
      NSString *msg = @"Attempting to use socket without a delegate queue. Set "
          @"a delegate queue first.";
      *errPtr = [self badConfigError:msg];
    }
    return NO;
  }
...
- (void)asyncResolveHost:(NSString *)aHost
  ...
  if (aHost == nil) {
    ...
    // We should still use dispatch_async since this method is expected to be
    // asynchronous
  ...
  // So we want to copy it now, within this block that will be executed
  // synchronously.
  // This way the asynchronous lookup block below doesn't have to worry about it
  // changing.
  ...
  dispatch_async(globalConcurrentQueue, ^{
      @autoreleasepool {
        ...
        if ([host isEqualToString:@"localhost"] ||
          ...
          [addresses addObject:[NSData dataWithBytes:&sockaddr4
                                              length:sizeof(sockaddr4)]];
          [addresses addObject:[NSData dataWithBytes:&sockaddr6
                                              length:sizeof(sockaddr6)]];
        } else {
          ...
          int gai_error = getaddrinfo([host UTF8String], [portStr UTF8String],
                                      &hints, &res0);
          ...
          if (gai_error) {
            ...
            for (res = res0; res; res = res->ai_next) {
              if (res->ai_family == AF_INET) {
                ...
                [addresses addObject:[NSData dataWithBytes:res->ai_addr
 ...
 * The address picked depends upon which protocols are disabled, deactived, &
*preferred.
 *
...
**/
- (int)getAddress:(NSData **)addressPtr
            error:(NSError **)errorPtr
    fromAddresses:(NSArray *)addresses {
  NSAssert(dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey),
           @"Must be dispatched on socketQueue");
  ...
  for (NSData *address in addresses) {
    switch ([[self class] familyFromAddress:address]) {
      case AF_INET:
        resolvedIPv4Address = YES;
        break;
      case AF_INET6:
        resolvedIPv6Address = YES;
        break;

      default:
        NSAssert(NO, @"Addresses array contains invalid address");
    }
  }
  ...
  if (isIPv4Deactivated && !resolvedIPv6Address) {
    NSString *msg = @"IPv4 has been deactivated due to bind/connect, and DNS "
        @"lookup found no IPv6 address(es).";
    resultError = [self otherError:msg];
  ...
  if (isIPv6Deactivated && !resolvedIPv4Address) {
    NSString *msg = @"IPv6 has been deactivated due to bind/connect, and DNS "
        @"lookup found no IPv4 address(es).";
    resultError = [self otherError:msg];
 ...
 * An inteface description may be an interface name (en0, en1, lo0) or
*corresponding IP (192.168.4.34).
**/
      ...
      while (cursor != NULL) {
        if ((addr4 == nil) && (cursor->ifa_addr->sa_family == AF_INET)) {
          ...
          if (strcmp(cursor->ifa_name, iface) == 0) {
            ...
            addr4 =
                [NSData dataWithBytes:&nativeAddr4 length:sizeof(nativeAddr4)];
          } else {
            ...
            if ((conversion != NULL) && (strcmp(ip, iface) == 0)) {
              ...
              addr4 = [NSData dataWithBytes:&nativeAddr4
                                     length:sizeof(nativeAddr4)];
            }
          }
        } else if ((addr6 == nil) &&
          ...
          if (strcmp(cursor->ifa_name, iface) == 0) {
            ...
            addr6 =
                [NSData dataWithBytes:&nativeAddr6 length:sizeof(nativeAddr6)];
          } else {
            ...
            if ((conversion != NULL) && (strcmp(ip, iface) == 0)) {
              ...
              addr6 = [NSData dataWithBytes:&nativeAddr6
                                     length:sizeof(nativeAddr6)];
            }
          }
        }
 ...
 * The hostname is expected to be an IPv4 or IPv6 address represented as a
*human-readable string. (e.g. 192.168.4.34)
**/
- (void)convertNumericHost:(NSString *)numericHost
  ...
  if (numericHost) {
    ...
    if (getaddrinfo([numericHost UTF8String], [portStr UTF8String], &hints,
                    &res0) == 0) {
      for (res = res0; res; res = res->ai_next) {
...
- (BOOL)isConnectedToAddress4:(NSData *)someAddr4 {
  ...
  const struct sockaddr_in *sSockaddr4 =
      (struct sockaddr_in *)[someAddr4 bytes];
  const struct sockaddr_in *cSockaddr4 =
      (struct sockaddr_in *)[cachedConnectedAddress bytes];

  if (memcmp(&sSockaddr4->sin_addr, &cSockaddr4->sin_addr,
             sizeof(struct in_addr)) != 0) {
    return NO;
  }
  if (memcmp(&sSockaddr4->sin_port, &cSockaddr4->sin_port, sizeof(in_port_t)) !=
      0) {
    return NO;
  }
...
- (BOOL)isConnectedToAddress6:(NSData *)someAddr6 {
  ...
  const struct sockaddr_in6 *sSockaddr6 =
      (struct sockaddr_in6 *)[someAddr6 bytes];
  const struct sockaddr_in6 *cSockaddr6 =
      (struct sockaddr_in6 *)[cachedConnectedAddress bytes];

  if (memcmp(&sSockaddr6->sin6_addr, &cSockaddr6->sin6_addr,
             sizeof(struct in6_addr)) != 0) {
    return NO;
  }
  if (memcmp(&sSockaddr6->sin6_port, &cSockaddr6->sin6_port,
             sizeof(in_port_t)) != 0) {
    return NO;
  }
  ...
  if ((getifaddrs(&addrs) == 0)) {
    ...
    while (cursor != NULL) {
      if (cursor->ifa_addr->sa_family == AF_INET) {
        ...
        if (memcmp(&addr->sin_addr, &ifaceAddr->sin_addr,
                   sizeof(struct in_addr)) == 0) {
...
- (unsigned int)indexOfInterfaceAddr6:(NSData *)interfaceAddr6 {
  ...
  struct sockaddr_in6 *ifaceAddr =
      (struct sockaddr_in6 *)[interfaceAddr6 bytes];
  ...
  if ((getifaddrs(&addrs) == 0)) {
    ...
    while (cursor != NULL) {
      if (cursor->ifa_addr->sa_family == AF_INET6) {
        ...
        if (memcmp(&addr->sin6_addr, &ifaceAddr->sin6_addr,
                   sizeof(struct in6_addr)) == 0) {
          result = if_nametoindex(cursor->ifa_name);
          break;
        }
...
- (void)setupSendAndReceiveSourcesForSocket4 {
  ...
  send4Source = dispatch_source_create(DISPATCH_SOURCE_TYPE_WRITE, socket4FD, 0,
                                       socketQueue);
  receive4Source = dispatch_source_create(DISPATCH_SOURCE_TYPE_READ, socket4FD,
                                          0, socketQueue);
  ...
  dispatch_source_set_event_handler(send4Source, ^{
      @autoreleasepool {
        ...
        // Otherwise pause the send source or it will continue to fire over and
        // over again.
...
- (void)setupSendAndReceiveSourcesForSocket6 {
  ...
  send6Source = dispatch_source_create(DISPATCH_SOURCE_TYPE_WRITE, socket6FD, 0,
                                       socketQueue);
  receive6Source = dispatch_source_create(DISPATCH_SOURCE_TYPE_READ, socket6FD,
                                          0, socketQueue);
...
- (BOOL)createSocket4:(BOOL)useIPv4
              socket6:(BOOL)useIPv6
  ...
  int (^createSocket)(int) = ^int(int domain) {
      ...
      int reuseaddr = 1;
      status = setsockopt(socketFD, SOL_SOCKET, SO_REUSEADDR, &reuseaddr,
                          sizeof(reuseaddr));
      if (status == -1) {
        if (errPtr)
          *errPtr = [self errnoErrorWithReason:
      ...
      int nosigpipe = 1;
      status = setsockopt(socketFD, SOL_SOCKET, SO_NOSIGPIPE, &nosigpipe,
                          sizeof(nosigpipe));
      if (status == -1) {
        if (errPtr)
          *errPtr = [self
              errnoErrorWithReason:@"Error disabling sigpipe (setsockopt)"];
  ...
  if (useIPv4) [self setupSendAndReceiveSourcesForSocket4];
  if (useIPv6) [self setupSendAndReceiveSourcesForSocket6];
...
- (void)closeSocket4 {
  if (socket4FD != SOCKET_NULL) {
    ...
    // For some crazy reason (in my opinion), cancelling a dispatch source
    // doesn't
    // invoke the cancel handler if the dispatch source is paused.
    // So we have to unpause the source if needed.
    // This allows the cancel handler to be run, which in turn releases the
    // source and closes the socket.

    [self resumeSend4Source];
    [self resumeReceive4Source];

    // The sockets will be closed by the cancel handlers of the corresponding
    // source
...
- (BOOL)getLocalAddress:(NSData **)dataPtr
  ...
  if (socketFamily == AF_INET) {
    ...
    if (getsockname(socketFD, (struct sockaddr *)&sockaddr4, &sockaddr4len) ==
        0) {
    ...
    if (getsockname(socketFD, (struct sockaddr *)&sockaddr6, &sockaddr6len) ==
        0) {
...
- (void)maybeUpdateCachedLocalAddress4Info {
  ...
  if (cachedLocalAddress4 || ((flags & kDidBind) == 0) ||
      (socket4FD == SOCKET_NULL)) {
    return;
  }
  ...
  if ([self getLocalAddress:&address
                       host:&host
                       port:&port
                  forSocket:socket4FD
                 withFamily:AF_INET]) {
...
- (void)maybeUpdateCachedLocalAddress6Info {
  ...
  if (cachedLocalAddress6 || ((flags & kDidBind) == 0) ||
      (socket6FD == SOCKET_NULL)) {
    return;
  }
...
- (void)maybeUpdateCachedConnectedAddressInfo {
  ...
  if (socket4FD != SOCKET_NULL) {
    ...
    if (getpeername(socket4FD, (struct sockaddr *)&sockaddr4, &sockaddr4len) ==
        0) {
      data = [NSData dataWithBytes:&sockaddr4 length:sockaddr4len];
      host = [[self class] hostFromSockaddr4:&sockaddr4];
      port = [[self class] portFromSockaddr4:&sockaddr4];
    ...
    if (getpeername(socket6FD, (struct sockaddr *)&sockaddr6, &sockaddr6len) ==
        0) {
      data = [NSData dataWithBytes:&sockaddr6 length:sockaddr6len];
      host = [[self class] hostFromSockaddr6:&sockaddr6];
      port = [[self class] portFromSockaddr6:&sockaddr6];
...
- (BOOL)isConnected {
  ...
  dispatch_block_t block = ^{ result = (flags & kDidConnect) ? YES : NO; };
...
- (BOOL)preBind:(NSError **)errPtr {
  ...
  {
    if (errPtr) {
      NSString *msg = @"Both IPv4 and IPv6 have been disabled. Must enable at "
          @"least one protocol first.";
      *errPtr = [self badConfigError:msg];
    }
    return NO;
  }
...
- (BOOL)bindToPort:(uint16_t)port interface:(NSString *)interface error:(NSError **)errPtr
{
  ...
  dispatch_block_t block = ^{
      @autoreleasepool {
        ...
        [self convertIntefaceDescription:
                  interface port:port
                            intoAddress4:&interface4
                                address6:&interface6];

        if ((interface4 == nil) && (interface6 == nil)) {
          NSString *msg = @"Unknown interface. Specify valid interface by name "
              @"(e.g. \"en1\") or IP address.";
        ...
        if (isIPv4Disabled && (interface6 == nil)) {
          NSString *msg = @"IPv4 has been disabled and specified interface "
        ...
        if (isIPv6Disabled && (interface4 == nil)) {
          NSString *msg = @"IPv6 has been disabled and specified interface "
        ...
        LogVerbose(@"Binding socket to port(%hu) interface(%@)", port,
                   interface);

        if (useIPv4) {
          int status = bind(socket4FD, (struct sockaddr *)[interface4 bytes],
        ...
        if (useIPv6) {
          int status = bind(socket6FD, (struct sockaddr *)[interface6 bytes],
  ...
  if (err) LogError(@"Error binding to port/interface: %@", err);
...
- (BOOL)bindToAddress:(NSData *)localAddr error:(NSError **)errPtr {
  ...
  dispatch_block_t block = ^{
      @autoreleasepool {
        ...
        if (useIPv4) {
          ...
          int status = bind(socket4FD, (struct sockaddr *)[localAddr4 bytes],
                            (socklen_t)[localAddr4 length]);
          if (status == -1) {
            [self closeSockets];
          ...
          int status = bind(socket6FD, (struct sockaddr *)[localAddr6 bytes],
                            (socklen_t)[localAddr6 length]);
          if (status == -1) {
            [self closeSockets];
  ...
  if (err) LogError(@"Error binding to address: %@", err);
...
/**
 * This method runs through the various checks required prior to a connect
*attempt.
...
- (BOOL)connectToHost:(NSString *)host
  ...
  dispatch_block_t block = ^{
      @autoreleasepool {
        ...
        if (host == nil) {
          NSString *msg = @"The host param is nil. Should be domain name or IP "
              @"address string.";
          err = [self badParamError:msg];
        ...
        GCDAsyncUdpSpecialPacket *packet =
            [[GCDAsyncUdpSpecialPacket alloc] init];
        ...
        [self asyncResolveHost:host
            ...
            withCompletionBlock:^(NSArray *addresses, NSError *error) {
                ...
                // The asyncResolveHost:port:: method asynchronously dispatches
                // a task onto the global concurrent queue,
                // and immediately returns. Once the async resolve task
                // completes,
  ...
  if (err) LogError(@"Error connecting to host/port: %@", err);
...
- (BOOL)connectToAddress:(NSData *)remoteAddr error:(NSError **)errPtr {
  ...
  if (err) LogError(@"Error connecting to address: %@", err);
...
- (void)maybeConnect {
  ...
  BOOL sendQueueReady =
      [currentSend isKindOfClass:[GCDAsyncUdpSpecialPacket class]];

  if (sendQueueReady) {
    GCDAsyncUdpSpecialPacket *connectPacket =
        (GCDAsyncUdpSpecialPacket *)currentSend;
    ...
    if (connectPacket->resolveInProgress) {
      ...
      if (connectPacket->error) {
        ...
        int addressFamily = [self getAddress:&address
                                       error:&error
                               fromAddresses:connectPacket->addresses];
...
- (BOOL)connectWithAddress4:(NSData *)address4 error:(NSError **)errPtr {
  ...
  int status = connect(socket4FD, (struct sockaddr *)[address4 bytes],
...
- (BOOL)connectWithAddress6:(NSData *)address6 error:(NSError **)errPtr {
  ...
  int status = connect(socket6FD, (struct sockaddr *)[address6 bytes],
                          ...
                          error:(NSError **)errPtr
{
  ...
  dispatch_block_t block = ^{
      @autoreleasepool {
        ...
        [self convertNumericHost:group
                            port:0
                    intoAddress4:&groupAddr4
                        address6:&groupAddr6];
        ...
        [self convertIntefaceDescription:
                  interface port:0
                            intoAddress4:&interfaceAddr4
                                address6:&interfaceAddr6];
        ...
        if ((socket4FD != SOCKET_NULL) && groupAddr4 && interfaceAddr4) {
          const struct sockaddr_in *nativeGroup =
              (struct sockaddr_in *)[groupAddr4 bytes];
          const struct sockaddr_in *nativeIface =
              (struct sockaddr_in *)[interfaceAddr4 bytes];

          struct ip_mreq imreq;
          imreq.imr_multiaddr = nativeGroup->sin_addr;
          imreq.imr_interface = nativeIface->sin_addr;

          int status = setsockopt(socket4FD, IPPROTO_IP, requestType,
          ...
          const struct sockaddr_in6 *nativeGroup =
              (struct sockaddr_in6 *)[groupAddr6 bytes];

          struct ipv6_mreq imreq;
          imreq.ipv6mr_multiaddr = nativeGroup->sin6_addr;
          imreq.ipv6mr_interface = [self indexOfInterfaceAddr6:interfaceAddr6];

          int status = setsockopt(socket6FD, IPPROTO_IPV6, requestType,
                                  (const void *)&imreq, sizeof(imreq));
          if (status != 0) {
            err = [self errnoErrorWithReason:@"Error in setsockopt() function"];
...
- (BOOL)enableBroadcast:(BOOL)flag error:(NSError **)errPtr {
  ...
  dispatch_block_t block = ^{
      @autoreleasepool {
        ...
        if (socket4FD != SOCKET_NULL) {
          int value = flag ? 1 : 0;
          int error = setsockopt(socket4FD, SOL_SOCKET, SO_BROADCAST,
        ...
        // IPv6 does not implement broadcast, the ability to send a packet to
        // all hosts on the attached link.
        // The same effect can be achieved by sending a packet to the link-local
...
- (void)sendData:(NSData *)data
  ...
  GCDAsyncUdpSendPacket *packet =
      [[GCDAsyncUdpSendPacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)sendData:(NSData *)data
  ...
  [self asyncResolveHost:host
      ...
      withCompletionBlock:^(NSArray *addresses, NSError *error) {
          ...
          // The asyncResolveHost:port:: method asynchronously dispatches a task
          // onto the global concurrent queue,
  ...
  if (currentSend == nil) {
    ...
    while ([sendQueue count] > 0) {
      ...
      if ([currentSend isKindOfClass:[GCDAsyncUdpSpecialPacket class]]) {
        ...
        return;  // The maybeConnect method, if it connects, will invoke this
                 // method again
      } else if (currentSend->resolveError) {
        // Notify delegate
        [self notifyDidNotSendDataWithTag:currentSend->tag
                               dueToError:currentSend->resolveError];
...
- (void)doPreSend {
  ...
  if (flags & kDidConnect) {
    ...
    if (currentSend->resolveInProgress || currentSend->resolvedAddresses ||
        currentSend->resolveError) {
      NSString *msg =
    ...
    if (currentSend->resolveInProgress) {
      ...
      if (currentSend->resolvedAddresses == nil) {
        NSString *msg = @"You must specify destination of packet for a "
            @"non-connected socket";
        error = [self badConfigError:msg];
      } else {
        // Pick the proper address to use (out of possibly several resolved
        // addresses)

        NSData *address = nil;
        int addressFamily = AF_UNSPEC;

        addressFamily = [self getAddress:&address
                                   error:&error
                           fromAddresses:currentSend->resolvedAddresses];
  ...
  if (sendFilterBlock && sendFilterQueue) {
    ...
    if (sendFilterAsync) {
      ...
      dispatch_async(sendFilterQueue, ^{
          @autoreleasepool {
            BOOL allowed = sendFilterBlock(
                sendPacket->buffer, sendPacket->address, sendPacket->tag);
                ...
                @autoreleasepool {
                  ...
                  if (sendPacket == currentSend) {
                    if (allowed) {
                      ...
                      LogVerbose(
                          @"currentSend - silently dropped by sendFilter");
      ...
      dispatch_sync(sendFilterQueue, ^{
          @autoreleasepool {
            allowed = sendFilterBlock(currentSend->buffer, currentSend->address,
                                      currentSend->tag);
          }
      });
...
- (void)doSendTimeout {
  ...
  [self notifyDidNotSendDataWithTag:currentSend->tag
                         dueToError:[self sendTimeoutError]];
  [self endCurrentSend];
  [self maybeDequeueSend];
}
...
- (BOOL)receiveOnce:(NSError **)errPtr {
      ...
      if ((flags & kReceiveOnce) == 0) {
        if ((flags & kDidCreateSockets) == 0) {
          NSString *msg = @"Must bind socket before you can receive data. "
              @"You can do this explicitly via bind, or implicitly via connect "
              @"or by sending data.";
  ...
  if (err) LogError(@"Error in beginReceiving: %@", err);
...
- (void)doReceive {
  ...
  if ((flags & kReceiveOnce) && (pendingFilterOperations > 0)) {
    LogVerbose(
        @"Receiving is temporarily paused (pending filter operations)...");
  ...
  if (flags & kDidConnect) {
    ...
    if (socket4FDBytesAvailable > 0) {
      if (socket6FDBytesAvailable > 0) {
        ...
        flags ^=
            kFlipFlop;  // flags = flags xor kFlipFlop; (toggle flip flop bit)
      } else {
  ...
  if (doReceive4) {
    ...
    result = recvfrom(socket4FD, buf, bufSize, 0, (struct sockaddr *)&sockaddr4,
                      &sockaddr4len);
    ...
    result = recvfrom(socket6FD, buf, bufSize, 0, (struct sockaddr *)&sockaddr6,
                      &sockaddr6len);
  ...
  if (result == 0) {
    ...
    if (flags & kDidConnect) {
      if (addr4 && ![self isConnectedToAddress4:addr4]) ignored = YES;
      if (addr6 && ![self isConnectedToAddress6:addr6]) ignored = YES;
    }
    ...
    if (!ignored) {
      if (receiveFilterBlock && receiveFilterQueue) {
        ...
        if (receiveFilterAsync) {
          ...
          dispatch_async(receiveFilterQueue, ^{
              @autoreleasepool {
                ...
                // Transition back to socketQueue to get the current delegate /
                // delegateQueue
                dispatch_async(socketQueue, ^{
                    @autoreleasepool {
                      pendingFilterOperations--;
                      ...
                      if (allowed) {
                        [self notifyDidReceiveData:data
                                       fromAddress:addr
                                 withFilterContext:filterContext];
                      } else {
                        LogVerbose(@"received packet silently dropped by "
                                   @"receiveFilter");
                      }
        ...
        {
          dispatch_sync(receiveFilterQueue, ^{
              @autoreleasepool {
                allowed = receiveFilterBlock(data, addr, &filterContext);
              }
          });
...
+ (void)listenerThread {
  @autoreleasepool {
    ...
    // We can't run the run loop unless it has an associated input source or a
    // timer.
    // So we'll just create a timer that will never fire - unless the server
    // runs for a decades.
...
+ (void)addStreamListener:(GCDAsyncUdpSocket *)asyncUdpSocket {
  ...
  if (asyncUdpSocket->readStream4)
    CFReadStreamScheduleWithRunLoop(asyncUdpSocket->readStream4, runLoop,
                                    kCFRunLoopDefaultMode);

  if (asyncUdpSocket->readStream6)
    CFReadStreamScheduleWithRunLoop(asyncUdpSocket->readStream6, runLoop,
                                    kCFRunLoopDefaultMode);

  if (asyncUdpSocket->writeStream4)
    CFWriteStreamScheduleWithRunLoop(asyncUdpSocket->writeStream4, runLoop,
                                     kCFRunLoopDefaultMode);

  if (asyncUdpSocket->writeStream6)
    CFWriteStreamScheduleWithRunLoop(asyncUdpSocket->writeStream6, runLoop,
                                     kCFRunLoopDefaultMode);
}
...
+ (void)removeStreamListener:(GCDAsyncUdpSocket *)asyncUdpSocket {
  ...
  if (asyncUdpSocket->readStream4)
    CFReadStreamUnscheduleFromRunLoop(asyncUdpSocket->readStream4, runLoop,
                                      kCFRunLoopDefaultMode);

  if (asyncUdpSocket->readStream6)
    CFReadStreamUnscheduleFromRunLoop(asyncUdpSocket->readStream6, runLoop,
                                      kCFRunLoopDefaultMode);

  if (asyncUdpSocket->writeStream4)
    CFWriteStreamUnscheduleFromRunLoop(asyncUdpSocket->writeStream4, runLoop,
                                       kCFRunLoopDefaultMode);

  if (asyncUdpSocket->writeStream6)
    CFWriteStreamUnscheduleFromRunLoop(asyncUdpSocket->writeStream6, runLoop,
                                       kCFRunLoopDefaultMode);
}

static void CFReadStreamCallback(CFReadStreamRef stream, CFStreamEventType type,
                                 void *pInfo) {
  @autoreleasepool {
    ...
    switch (type) {
      ...
      case kCFStreamEventEndEncountered: {
        NSError *error =
            (__bridge_transfer NSError *)CFReadStreamCopyError(stream);
...
static void CFWriteStreamCallback(CFWriteStreamRef stream,
  ...
  @autoreleasepool {
    ...
    switch (type) {
      ...
      case kCFStreamEventEndEncountered: {
        NSError *error =
            (__bridge_transfer NSError *)CFWriteStreamCopyError(stream);
        if (error == nil && type == kCFStreamEventEndEncountered) {
          error = [asyncUdpSocket socketClosedError];
        }
...
- (BOOL)createReadAndWriteStreams:(NSError **)errPtr {
  ...
  if (socket4FD != SOCKET_NULL) {
    CFStreamCreatePairWithSocket(NULL, (CFSocketNativeHandle)socket4FD,
                                 &readStream4, &writeStream4);
  ...
  if (socket6FD != SOCKET_NULL) {
    CFStreamCreatePairWithSocket(NULL, (CFSocketNativeHandle)socket6FD,
                                 &readStream6, &writeStream6);
  ...
  CFReadStreamSetProperty(readStream4, kCFStreamPropertyShouldCloseNativeSocket,
                          kCFBooleanFalse);
  CFWriteStreamSetProperty(
      writeStream4, kCFStreamPropertyShouldCloseNativeSocket, kCFBooleanFalse);

  CFReadStreamSetProperty(readStream6, kCFStreamPropertyShouldCloseNativeSocket,
                          kCFBooleanFalse);
  CFWriteStreamSetProperty(
      writeStream6, kCFStreamPropertyShouldCloseNativeSocket, kCFBooleanFalse);
...
- (BOOL)registerForStreamCallbacks:(NSError **)errPtr {
  ...
  //	readStreamEvents  |= (kCFStreamEventOpenCompleted |
  // kCFStreamEventHasBytesAvailable);
  //	writeStreamEvents |= (kCFStreamEventOpenCompleted |
  ...
  if (socket4FD != SOCKET_NULL) {
    ...
    BOOL r1 = CFReadStreamSetClient(readStream4, readStreamEvents,
                                    &CFReadStreamCallback, &streamContext);
    BOOL r2 = CFWriteStreamSetClient(writeStream4, writeStreamEvents,
                                     &CFWriteStreamCallback, &streamContext);
  ...
  if (socket6FD != SOCKET_NULL) {
    ...
    BOOL r1 = CFReadStreamSetClient(readStream6, readStreamEvents,
                                    &CFReadStreamCallback, &streamContext);
    BOOL r2 = CFWriteStreamSetClient(writeStream6, writeStreamEvents,
                                     &CFWriteStreamCallback, &streamContext);
...
- (void)applicationWillEnterForeground:(NSNotification *)notification {
  ...
  // If the application was backgrounded, then iOS may have shut down our
  // sockets.
...
- (void)unmarkSocketQueueTargetQueue:(dispatch_queue_t)socketOldTargetQueue {
  dispatch_queue_set_specific(socketOldTargetQueue,
                              IsOnSocketQueueOrTargetQueueKey, NULL, NULL);
}
...
- (int)socketFD {
  if (!dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey)) {
    LogWarn(@"%@: %@ - Method only available from within the context of a "
            @"performBlock: invocation",
...
- (BOOL)enableBackgroundingOnSockets {
  ...
  //	if (readStream4 && writeStream4)
  //	{
  //		r1 = CFReadStreamSetProperty(readStream4,
  // kCFStreamNetworkServiceType, kCFStreamNetworkServiceTypeVoIP);
  //		r2 = CFWriteStreamSetProperty(writeStream4,
  // kCFStreamNetworkServiceType, kCFStreamNetworkServiceTypeVoIP);
  //
  //		if (!r1 || !r2)
  //		{
  ...
  //	if (readStream6 && writeStream6)
  //	{
  //		r1 = CFReadStreamSetProperty(readStream6,
  // kCFStreamNetworkServiceType, kCFStreamNetworkServiceTypeVoIP);
  //		r2 = CFWriteStreamSetProperty(writeStream6,
  // kCFStreamNetworkServiceType, kCFStreamNetworkServiceTypeVoIP);
  //
  //		if (!r1 || !r2)
  //		{
  ...
  //
  // One tiny problem: the sockets will still get closed when the app gets
  // backgrounded.
  //
...
+ (NSString *)hostFromSockaddr4:(const struct sockaddr_in *)pSockaddr4 {
  ...
  if (inet_ntop(AF_INET, &pSockaddr4->sin_addr, addrBuf,
...
+ (NSString *)hostFromSockaddr6:(const struct sockaddr_in6 *)pSockaddr6 {
  ...
  if (inet_ntop(AF_INET6, &pSockaddr6->sin6_addr, addrBuf,
                (socklen_t)sizeof(addrBuf)) == NULL) {
    addrBuf[0] = '\0';
  }
