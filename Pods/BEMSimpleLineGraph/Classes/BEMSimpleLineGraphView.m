...
// Add the -fobjc-arc flag to enable ARC for only these files, as described in
...
/// The X offset necessary to compensate the labels on the Y-Axis. Will take the
/// value of the bigger label on the Y-Axis
...
#pragma clang diagnostic pop
  ...
  if (numberOfPoints == 0) {
    ...
    self.noDataLabel = [[UILabel alloc]
    ...
    BEMCircle *circleDot = [[BEMCircle alloc]
        initWithFrame:CGRectMake(0, 0, self.sizePoint, self.sizePoint)];
    circleDot.center =
...
- (void)layoutTouchReport {
  ...
  if (self.enableTouchReport == YES || self.enablePopUpReport == YES) {
    ...
    // graph.
    self.touchInputLine =
        [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.widthTouchInputLine,
    ...
    if (self.enablePopUpReport == YES && self.alwaysDisplayPopUpLabels == NO) {
      ...
      NSString *maxValueString = [NSString
          stringWithFormat:@"%li", (long)[self calculateMaximumPointValue]
                                       .integerValue];
      NSString *minValueString = [NSString
          stringWithFormat:@"%li", (long)[self calculateMinimumPointValue]
                                       .integerValue];
      ...
      if ([self.delegate
        ...
        self.popUpLabel.text = [NSString
            stringWithFormat:@"%@%@", longestString,
      ...
      self.popUpView = [[UIView alloc]
          initWithFrame:CGRectMake(0, 0, self.popUpLabel.frame.size.width + 7,
  ...
  if (self.enableYAxisLabel) {
    ...
    if (self.autoScaleYAxis == YES) {
      NSString *maxValueString =
          [NSString stringWithFormat:@"%i", (int)[self maxValue]];
      NSString *minValueString =
          [NSString stringWithFormat:@"%i", (int)[self minValue]];

      self.YAxisLabelXOffset =
          MAX([maxValueString sizeWithAttributes:attributes].width,
              [minValueString sizeWithAttributes:attributes].width) +
          5;
    } else {
...
#pragma clang diagnostic pop
      ...
      positionOnXAxis = (((self.frame.size.width - self.YAxisLabelXOffset) /
                          (numberOfPoints - 1)) *
                         i) +
                        self.YAxisLabelXOffset;
      ...
      if (self.animationGraphEntranceTime != 0 ||
        ...
        if (self.animationGraphEntranceTime == 0) {
          ...
          [UIView animateWithDuration:(float)self.animationGraphEntranceTime /
                                      numberOfPoints
              delay:(float)i *
                    ((float)self.animationGraphEntranceTime / numberOfPoints)
              options:UIViewAnimationOptionCurveLinear
              animations:^{ circleDot.alpha = 0.7; }
...
- (void)drawLine {
  ...
  BEMLine *line = [[BEMLine alloc]
      initWithFrame:CGRectMake(self.YAxisLabelXOffset, 0,
                               self.frame.size.width - self.YAxisLabelXOffset,
                               self.frame.size.height)];
  line.opaque = NO;
  line.alpha = 1;
  ...
  if (self.enableReferenceXAxisLines || self.enableReferenceYAxisLines) {
    ...
    line.arrayOfVerticalRefrenceLinePoints =
        self.enableReferenceXAxisLines ? xAxisLabelPoints : nil;
    line.arrayOfHorizontalRefrenceLinePoints =
        self.enableReferenceYAxisLines ? yAxisLabelPoints : nil;
  }
  ...
  if ([self.delegate
    ...
    numberOfGaps =
        [self.delegate numberOfGapsBetweenLabelsOnLineGraph:self] + 1;
                 ...
                 respondsToSelector:@selector(numberOfGapsBetweenLabels)]) {
    [self printDeprecationWarningForOldMethod:@"numberOfGapsBetweenLabels"
                         andReplacementMethod:
                             @"numberOfGapsBetweenLabelsOnLineGraph:"];
...
#pragma clang diagnostic pop
  ...
  if (numberOfGaps >= (numberOfPoints - 1)) {
    ...
    if ([self.dataSource
      ...
      lastXLabel = [self.dataSource lineGraph:self
                         labelOnXAxisForIndex:(numberOfPoints - 1)];

    } else if ([self.delegate
                   respondsToSelector:@selector(labelOnXAxisForIndex:)]) {
      [self printDeprecationWarningForOldMethod:@"labelOnXAxisForIndex:"
                           andReplacementMethod:
...
#pragma clang diagnostic pop
    ...
    UILabel *firstLabel =
        [[UILabel alloc] initWithFrame:CGRectMake(3 + self.YAxisLabelXOffset,
    ...
    NSNumber *xFirstAxisLabelCoordinate =
        [NSNumber numberWithFloat:firstLabel.center.x - self.YAxisLabelXOffset];
    [xAxisLabelPoints addObject:xFirstAxisLabelCoordinate];

    UILabel *lastLabel = [[UILabel alloc]
        initWithFrame:CGRectMake(self.frame.size.width / 2 - 3,
                                 self.frame.size.height - 20,
    ...
    NSNumber *xLastAxisLabelCoordinate =
        [NSNumber numberWithFloat:lastLabel.center.x - self.YAxisLabelXOffset];
    ...
    @autoreleasepool {
      ...
      for (int i = 1; i <= (numberOfPoints / numberOfGaps); i++) {
        ...
        if ([self.dataSource respondsToSelector:@selector(lineGraph:
                                                    labelOnXAxisForIndex:)]) {
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
          xAxisLabelText = [self.delegate
              labelOnXAxisForIndex:(i * numberOfGaps - 1 - offset)];
#pragma clang diagnostic pop
        ...
        // Add support multi-line, but this might overlap with the graph line if
        // text have too many lines
      ...
      UILabel *firstXLabel = [xAxisLabels firstObject];
      firstXLabel.frame = CGRectMake(
          firstXLabel.frame.origin.x + firstXLabel.frame.size.width / 2,
          firstXLabel.frame.origin.y, firstXLabel.frame.size.width,
          firstXLabel.frame.size.height);
      UILabel *lastXLabel = [xAxisLabels lastObject];
      lastXLabel.frame = CGRectMake(
          lastXLabel.frame.origin.x - lastXLabel.frame.size.width / 2,
          lastXLabel.frame.origin.y, lastXLabel.frame.size.width,
          lastXLabel.frame.size.height);
...
- (void)drawYAxis {
  ...
  UIView *backgroundYaxis =
      [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.YAxisLabelXOffset,
                                               self.frame.size.height)];
  ...
  if (self.autoScaleYAxis) {
    ...
    NSNumber *minimumValue = [NSNumber
        numberWithInteger:[self calculateMinimumPointValue].integerValue];
    NSNumber *maximumValue = [NSNumber
        numberWithInteger:[self calculateMaximumPointValue].integerValue];
    ...
    NSMutableArray *dotValues = [[NSMutableArray alloc]
        initWithObjects:minimumValue, maximumValue, nil];
    ...
    for (NSNumber *dotValue in dotValues) {
      ...
      UILabel *labelYAxis = [[UILabel alloc]
          initWithFrame:CGRectMake(0, 0, self.YAxisLabelXOffset - 5, 15)];
      ...
      labelYAxis.tag = 2000;
      labelYAxis.center =
          CGPointMake(self.YAxisLabelXOffset / 2, yAxisPosition);
      [self addSubview:labelYAxis];
      [yAxisLabels addObject:labelYAxis];

      NSNumber *yAxisLabelCoordinate =
          [NSNumber numberWithFloat:labelYAxis.center.y];
    ...
    for (NSInteger i = numberOfLabels; i > 0; i--) {
      ...
      UILabel *labelYAxis = [[UILabel alloc]
          initWithFrame:CGRectMake(0, 0, self.YAxisLabelXOffset - 5, 10)];
      labelYAxis.center =
          CGPointMake(self.YAxisLabelXOffset / 2, yAxisPosition);
      labelYAxis.text = [NSString
          stringWithFormat:@"%i", (int)(graphHeight - self.XAxisLabelYOffset -
                                        yAxisPosition)];
      labelYAxis.font = self.labelFont;
      labelYAxis.textAlignment = NSTextAlignmentRight;
      labelYAxis.textColor = self.colorYaxisLabel;
...
- (NSInteger)offsetForXAxisWithNumberOfGaps:(NSInteger)numberOfGaps {
  // Calculates the optimum offset needed for the Labels to be centered on the
  // X-Axis.
  NSInteger leftGap = numberOfGaps - 1;
  NSInteger rightGap =
      numberOfPoints - (numberOfGaps * (numberOfPoints / numberOfGaps));
  NSInteger offset = 0;
...
- (UIImage *)graphSnapshotImage {
  ...
  [self drawViewHierarchyInRect:self.bounds
             afterScreenUpdates:YES]; // Pre-iOS 7 Style [self.layer
...
- (NSNumber *)calculatePointValueAverage {
  NSExpression *expression = [NSExpression
      expressionForFunction:@"average:"
                  arguments:@[
                    [NSExpression expressionForConstantValue:dataPoints]
                  ]];
...
- (void)handlePan:(UIPanGestureRecognizer *)recognizer {
  ...
  if (!((translation.x + self.frame.origin.x) <= self.frame.origin.x) &&
      !((translation.x + self.frame.origin.x) >=
        self.frame.origin.x +
            self.frame.size.width)) { // To make sure the vertical line doesn't
                                      // go beyond the frame of the graph.
    self.touchInputLine.frame =
        CGRectMake(translation.x - self.widthTouchInputLine / 2, 0,
                   self.widthTouchInputLine, self.frame.size.height);
  }
  ...
  if (self.enablePopUpReport == YES && closestDot.tag > 99 &&
      closestDot.tag < 1000 && [closestDot isKindOfClass:[BEMCircle class]] &&
      self.alwaysDisplayPopUpLabels == NO) {
    [self setUpPopUpLabelAbovePoint:closestDot];
  }

  if (closestDot.tag > 99 && closestDot.tag < 1000 &&
      [closestDot isMemberOfClass:[BEMCircle class]]) {
    if ([self.delegate
            respondsToSelector:@selector(lineGraph:
                                   didTouchGraphWithClosestIndex:)] &&
        self.enableTouchReport == YES) {
      [self.delegate lineGraph:self
          didTouchGraphWithClosestIndex:((NSInteger)closestDot.tag - 100)];
               ...
               self.enableTouchReport == YES) {
      [self
          printDeprecationWarningForOldMethod:@"didTouchGraphWithClosestIndex:"
                         andReplacementMethod:
                             @"lineGraph:didTouchGraphWithClosestIndex:"];
...
#pragma clang diagnostic pop
  ...
  if (recognizer.state == UIGestureRecognizerStateEnded) {
    if ([self.delegate respondsToSelector:
                           @selector(lineGraph:
                               didReleaseTouchFromGraphWithClosestIndex:)]) {
      [self.delegate lineGraph:self
          didReleaseTouchFromGraphWithClosestIndex:(closestDot.tag - 100)];

    } else if ([self.delegate
                   respondsToSelector:@selector(
                                          didReleaseGraphWithClosestIndex:)]) {
      [self printDeprecationWarningForOldMethod:
                @"didReleaseGraphWithClosestIndex:"
                           andReplacementMethod:@"lineGraph:"
                           @"didReleaseTouchFromGraphWith" @"ClosestIndex:"];
...
- (void)setUpPopUpLabelAbovePoint:(BEMCircle *)closestPoint {
  ...
  self.yCenterLabel =
      closestDot.center.y - closestDot.frame.size.height / 2 - 15;
  ...
  if ([self.delegate respondsToSelector:@selector(popUpSuffixForlineGraph:)])
    self.popUpLabel.text = [NSString
        stringWithFormat:@"%li%@",
                         (long)[[dataPoints
                             objectAtIndex:((NSInteger)closestDot.tag -
                                            100)] integerValue],
                         [self.delegate popUpSuffixForlineGraph:self]];
  else
    self.popUpLabel.text = [NSString
        stringWithFormat:@"%li", (long)[[dataPoints
                                     objectAtIndex:((NSInteger)closestDot.tag -
                                                    100)] integerValue]];
  if (self.enableYAxisLabel == YES &&
      self.popUpView.frame.origin.x <= self.YAxisLabelXOffset) {
    self.xCenterLabel = self.popUpView.frame.size.width / 2;
    self.popUpView.center = CGPointMake(
        self.xCenterLabel + self.YAxisLabelXOffset + 1, self.yCenterLabel);
  } else if (self.popUpView.frame.origin.x <= 0) {
    self.xCenterLabel = self.popUpView.frame.size.width / 2;
    self.popUpView.center = CGPointMake(self.xCenterLabel, self.yCenterLabel);
  } else if ((self.popUpView.frame.origin.x +
              self.popUpView.frame.size.width) >= self.frame.size.width) {
    self.xCenterLabel =
        self.frame.size.width - self.popUpView.frame.size.width / 2;
    self.popUpView.center = CGPointMake(self.xCenterLabel, self.yCenterLabel);
  }
...
- (BEMCircle *)closestDotFromtouchInputLine:(UIView *)touchInputLine {
  ...
  for (BEMCircle *point in self.subviews) {
    if (point.tag > 99 && point.tag < 1000 &&
        [point isMemberOfClass:[BEMCircle class]]) {
      if (self.alwaysDisplayDots == NO) {
        point.alpha = 0;
      }
      ...
      for (int i = 0; i < numberOfPoints; i++) {
        if ([self.dataSource respondsToSelector:@selector(lineGraph:
          ...
          dotValue = [self.dataSource lineGraph:self valueForPointAtIndex:i];
                       ...
                       respondsToSelector:@selector(valueForIndex:)]) {
          [self printDeprecationWarningForOldMethod:@"valueForIndex:"
                               andReplacementMethod:
                                   @"lineGraph:valueForPointAtIndex:"];
...
- (CGFloat)yPositionForDotValue:(CGFloat)dotValue {
  CGFloat maxValue =
      [self maxValue]; // Biggest Y-axis value from all the points.
  CGFloat minValue =
      [self minValue]; // Smallest Y-axis value from all the points.
