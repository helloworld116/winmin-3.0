  ...
  if (self = [super initWithOptions:options]) {
    ...
    if ([options objectForKey:nciUseDateFormatter])
      _nciUseDateFormatter =
          [[options objectForKey:nciUseDateFormatter] boolValue];
  ...
  if (self.vertical) {
    ...
    for (int i = 0; i <= length / _labelsDistance; i++) {
      float yPos = length - i * _labelsDistance - _labelsDistance / 2 +
                   self.chart.nciGridTopMargin;
      double curVal = ceil([self.chart.graph getValByY:_labelsDistance * i]);
      UILabel *label;
      if (_invertedLabes) {
        label = [[UILabel alloc]
            initWithFrame:CGRectMake(_nciAxisShift +
                                         self.chart.nciGridLeftMargin + 4,
                                     yPos, _labelWidth, _labelHeight)];
        label.textAlignment = NSTextAlignmentLeft;
      } else {
        label = [[UILabel alloc]
            initWithFrame:CGRectMake(0, yPos, _labelWidth - 4, _labelHeight)];
    ...
    for (int i = 0; i < length / _labelsDistance; i++) {
      ...
      if (xPos > (length + self.chart.nciGridLeftMargin - _labelsDistance / 2))
        continue;
      float yPos =
          (_nciAxisShift != _nciAxisShift)
              ? self.chart.graph.frame.size.height + self.chart.nciGridTopMargin
              : _nciAxisShift + self.chart.nciGridTopMargin;
      UILabel *label;
      if (_invertedLabes) {
        label = [[UILabel alloc]
            initWithFrame:CGRectMake(xPos, yPos - _labelHeight, _labelWidth,
                                     _labelHeight)];
      } else {
        label = [[UILabel alloc]
            initWithFrame:CGRectMake(xPos, yPos, _labelWidth, _labelHeight)];
      }
      double curVal = [self.chart.graph
          getArgumentByX:(_labelsDistance * i + _labelWidth / 2)];
...
- (void)makeUpLabel:(UILabel *)label val:(double)curVal {
  ...
  if (_labelRenderer) {
    ...
    label.text = [_dateFormatter
        stringFromDate:[NSDate dateWithTimeIntervalSince1970:curVal]];
  } else {
