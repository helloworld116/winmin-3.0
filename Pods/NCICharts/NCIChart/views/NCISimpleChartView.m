...
- (void)defaultSetup {
  ...
  _nciLineWidths = @[ @0.3 ];
  _nciLineColors = [NSMutableArray arrayWithArray:@[
    [UIColor blueColor],
    [UIColor greenColor],
    [UIColor purpleColor]
  ]];
  _nciSelPointColors = [NSMutableArray arrayWithArray:@[
    [UIColor blueColor],
    [UIColor greenColor],
    [UIColor purpleColor]
  ]];
...
- (id)initWithFrame:(CGRect)frame andOptions:(NSDictionary *)opts {
  ...
  if (self) {
    ...
    for (NSString *key in @[
           ...
           nciIsFill,
           nciIsSmooth,
           nciLeftRangeImageName,
           nciRightRangeImageName,
           nciLineWidths,
           nciSelPointColors,
           nciSelPointSizes,
           nciSelPointImages,
    ...
    _yAxis.chart = self;
    _nciGridVertical =
        [[NCILine alloc] initWithOptions:[opts objectForKey:nciGridVertical]];
    _nciGridHorizontal =
        [[NCILine alloc] initWithOptions:[opts objectForKey:nciGridHorizontal]];
    ...
    if ([opts objectForKey:nciGridBottomMargin]) {
      _nciGridBottomMargin =
          [[opts objectForKey:nciGridBottomMargin] floatValue];
    }

    if ([opts objectForKey:nciUseDateFormatter]) {
      _nciUseDateFormatter =
          [[opts objectForKey:nciUseDateFormatter] boolValue];
    } else {
    ...
    if ([opts objectForKey:nciHasHorizontalGrid]) {
      self.nciHasHorizontalGrid =
          [[opts objectForKey:nciHasHorizontalGrid] boolValue];
    }

    if ([opts objectForKey:nciXDiffMin]) {
      self.nciXDiffMin = [[opts objectForKey:nciXDiffMin] floatValue];
    } else {
      self.nciXDiffMin = 30.f;
    }
...
- (UIView *)createSelPoint:(int)num {
  ...
  if (num < [_nciSelPointSizes count] &&
      ![_nciSelPointSizes[num] isKindOfClass:[[NSNull null] class]]) {
    curSize = [_nciSelPointSizes[num] floatValue];
  }
  if (_nciSelPointImages && _nciSelPointImages.count >= (num + 1) &&
      ![_nciSelPointImages[num] isKindOfClass:[NSNull class]]) {
    selectedPoint =
        [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, curSize, curSize)];
    ((UIImageView *)selectedPoint).image =
        [UIImage imageNamed:_nciSelPointImages[num]];
  } else {
    selectedPoint =
        [[UIView alloc] initWithFrame:CGRectMake(0, 0, curSize, curSize)];
...
- (void)gridTapped:(UITapGestureRecognizer *)recognizer {
  ...
  if (self.nciTapGridAction) {
    self.nciTapGridAction([self.graph getArgumentByX:location.x],
                          [self.graph getValByY:location.y], location.x,
                          location.y);
  }
  [self layoutSelectedPoint];
}
  ...
  for (int i = 0; i < _chartData.count; i++) {
    ...
    if (selectedPointArgument <= [point[0] doubleValue]) {
      for (int j = 0; j < ((NSArray *)currentPoint[1]).count; j++) {
        ...
        CGPoint pointInGrid =
            [self.graph pointByValueInGrid:@[ currentPoint[0], val ]];
        if (prevPoint && prevPoint[1][j] &&
            ![prevPoint[1][j] isKindOfClass:[NSNull class]]) {
          if (([point[0] doubleValue] - selectedPointArgument) >
              (selectedPointArgument - [prevPoint[0] doubleValue])) {
            pointInGrid = [self.graph
                pointByValueInGrid:@[ prevPoint[0], prevPoint[1][j] ]];
        ...
        if (selectedPoints.count < (j + 1)) {
          ...
          if (!_nciSelPointImages || _nciSelPointImages.count <= j ||
              [_nciSelPointImages[j] isKindOfClass:[NSNull class]]) {
            if (_nciSelPointColors.count > j &&
                ![_nciSelPointColors[j] isKindOfClass:[NSNull class]]) {
              selectedPoint.backgroundColor = _nciSelPointColors[j];
            } else {
              UIColor *newColor =
                  [UIColor colorWithRed:(arc4random() % 255) / 255.0f
        ...
        selectedPoint.center =
            CGPointMake(pointInGrid.x + self.graph.chart.nciGridLeftMargin,
                        pointInGrid.y + _nciGridTopMargin);
        if (pointInGrid.x < 0 ||
            pointInGrid.x >= (self.graph.grid.frame.size.width + 2)) {
          selectedPoint.hidden = YES;
        } else {
          selectedPoint.hidden = NO;
        }
      ...
      if (self.nciSelPointTextRenderer) {
        NSObject *text = self.nciSelPointTextRenderer(
            [currentPoint[0] doubleValue], currentPoint[1]);
        ...
        if (_nciUseDateFormatter) {
          _selectedLabel.text = [NSString
              stringWithFormat:
                  @"y: %@  x:%@", values,
                  [dateFormatter
                      stringFromDate:[NSDate
                                         dateWithTimeIntervalSince1970:
                                             [currentPoint[0] doubleValue]]]];
        } else {
          _selectedLabel.text =
              [NSString stringWithFormat:@"y: %@  x:% 0.1f", values,
                                         [currentPoint[0] doubleValue]];
        }
      }
      return;
...
- (void)layoutSubviews {
  _graph.frame = CGRectMake(
      _nciGridLeftMargin, _nciGridTopMargin,
      self.bounds.size.width - _nciGridLeftMargin - _nciGridRightMargin,
      self.bounds.size.height - _nciGridTopMargin - _nciGridBottomMargin);
  //[_graph layoutSubviews];
  if (_nciHasSelection) {
    _selectedLabel.frame =
        CGRectMake(0, 0, self.bounds.size.width, _nciGridTopMargin);
    ;
    [self layoutSelectedPoint];
  }
}
...
- (NSArray *)getBoundaryValues {
  ...
  return @[
    @(minY - diff * _topBottomGridSpace / 100),
    @(maxY + diff * _topBottomGridSpace / 100)
  ];
}
