...
- (id)initWithFrame:(CGRect)frame {
  ...
  if (self) {
    UIPinchGestureRecognizer *croperViewGessture =
        [[UIPinchGestureRecognizer alloc]
            initWithTarget:self
                    action:@selector(croperViewScale:)];
...
- (void)croperViewScale:(id)sender {
  if (self.chart.chartData.count == 0)
    return;
  if ([(UIPinchGestureRecognizer *)sender state] ==
      UIGestureRecognizerStateBegan) {

    if ([sender numberOfTouches] == 2) {
      CGPoint point1 =
          [(UIPinchGestureRecognizer *)sender locationOfTouch:0 inView:self];
      CGPoint point2 =
          [(UIPinchGestureRecognizer *)sender locationOfTouch:1 inView:self];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
  ...
  self.grid.frame = CGRectMake(gridScroll.contentOffset.x, 0, self.gridWidth,
                               self.gridHeigth);
...
- (void)layoutSubviews {
  ...
  float timeDiff = [[self.chart.chartData lastObject][0] doubleValue] -
                   [self.chart.chartData[0][0] doubleValue];
  if (timeDiff == 0)
    timeDiff = 1000 * 60 * 2;
  ...
  double timeOffest;
  if (self.chart.xAxis.nciAxisDecreasing) {
    timeOffest = [[self.chart.chartData lastObject][0] doubleValue] -
                 self.chart.maxRangeVal;
  } else {
    timeOffest =
        self.chart.minRangeVal - [self.chart.chartData[0][0] doubleValue];
  }
  ...
  self.grid.frame =
      CGRectMake(timeOffest * stepX, 0, self.gridWidth, self.gridHeigth);
...
- (NSArray *)getValsInRanges {
  ...
  for (index = 1; index < self.chart.chartData.count; index++) {
    ...
    for (int serisNum = 0; serisNum < ((NSArray *)point[1]).count; serisNum++) {
      ...
      if ([point[1][serisNum] floatValue] >
          [prevPoint[1][serisNum] floatValue]) {
      ...
      if (self.chart.minRangeVal <= [point[0] doubleValue] &&
          (minYVal == MAXFLOAT ||
           self.chart.maxRangeVal >= [point[0] doubleValue])) {
  ...
  return @[ @(minYVal), @(maxYVal), @(firstDataIndex), @(lastChartIndex) ];
}

- (double)getScaleIndex {
  if (self.chart.minRangeVal != self.chart.minRangeVal ||
      self.chart.maxRangeVal != self.chart.maxRangeVal)
    return 1;
...
- (double)getXValuesGap {
  if (self.chart.chartData.count == 0)
    return 0;
  return [[self.chart.chartData lastObject][0] doubleValue] -
         [self.chart.chartData[0][0] doubleValue];
}
...
- (double)getArgumentByX:(float)pointX {
  ...
  if (self.chart.xAxis.nciAxisDecreasing) {
    return self.maxXVal -
           (gridScroll.contentOffset.x + pointX) / scaleIndex / self.xStep;
  } else {
    return self.minXVal +
           (gridScroll.contentOffset.x + pointX) / scaleIndex / self.xStep;
  }
